...
export class InfractorService {
  ...
  actualizarInfractor(item: Infractor) {
    this.appService.putRequest(`${Constants.PATH_INFRACTORES}/${item.identificacion}`, item).then((resp) => {
  ...
  eliminarInfractor(item: Infractor) {
    this.alertService.confirm("¿Está seguro que desea eliminar la infractor?", opction => {
      if (opction) {
        this.appService.deleteRequest(Constants.PATH_INFRACTORES, `${item.identificacion}`).then((resp) => {
