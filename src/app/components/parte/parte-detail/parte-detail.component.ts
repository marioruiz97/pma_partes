...
export class ParteDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.infractorService.cargarInfractores();
  ...
  saveParte() {
    ...
    this.parte.infractor = this.infractorService.infractores.filter((item : Infractor) => item.identificacion === +this.identificacion)[0];
    this.parte.usuario = this.usuarioService.usuarios.filter((item : Usuario) => item.identificacion === +this.usuario)[0];
    debugger;
